...
Функция Проведение ( Данные ) Экспорт
	...
	Если ( Данные.Шапка.ВидОперации = Перечисления.ВидОперацииПриходныйКассовыйОрдер.Конвертация ) Тогда
		// код ...
	ИначеЕсли ( Данные.Шапка.ВидОперации = Перечисления.ВидОперацииПриходныйКассовыйОрдер.ПеремещениеИзКассы ) Тогда 
		// код ...
	Иначе
...
Процедура получитьДанныеДокумента ( Данные )
	...
	|	СуммаДоллар как СуммаДоллар, ВидОперации как ВидОперации,
	...
	|из
	|	Документ.ПриходныйКассовыйОрдер.Транспорт
	|где
	|	Ссылка = &Ссылка
	|упорядочить по
	|	НомерСтроки
	...
	Данные.Вставить ( "Оплаты", результат [ 3 ].Выгрузить () );
...
Процедура сформироватьДенежныеСредства ( Данные )
	...
	п = Данные.Шапка;
	Если ( Данные.Шапка.ВидОперации = Перечисления.ВидОперацииПриходныйКассовыйОрдер.ПеремещениеИзКассы ) Тогда
...
Процедура сформироватьДвиженияДенежныхСредств ( Данные )
	...
	Иначе
		Для Каждого строкаТЗ Из Данные.Оплаты Цикл
			...
			движение.СтатьяДДС = п.СтатьяДДС;
			движение.Контрагент = п.Контрагент;
			движение.СуммаПриход = строкаТЗ.Сумма;
			движение.СуммаРасход = 0;
		КонецЦикла;
	КонецЕсли;
